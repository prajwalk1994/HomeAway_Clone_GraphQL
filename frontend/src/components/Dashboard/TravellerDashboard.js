...
import {travellerProperties} from '../../queries/dashboardQueries';
...
class TravellerDashboard extends Component{
    ...
    componentWillMount(){
      let traveller = cookie.load('travellerCookie');
      this.props.client.query({
        query : travellerProperties,
        variables : {
          email : traveller,
        }
      }).then((response) => {
        ...
        this.setState({
          properties : this.state.properties.concat(response.data.travellerProperties.bookedProperties)
        })
      })
